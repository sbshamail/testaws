...
const UserSettingMenu = () => {
  ...
  const handleClearCache = () => {
    ls.deleteAll();
    ls.deleteAll("dealerSettings");
    ls.deleteAll("eventLogs");
  };
  return (
        ...
        <IconDropdown
          ...
          customIcon={(open) => {
            return (
              <div className="flex items-center group p-2 transition-all duration-500 relative rounded-2xl overflow-hidden">
                ...
                >
                  <Image
                    src={GLOBAL_RESPONSE?.UserImage || "/images/Avatar.png"}
                    height={100}
                    width={100}
                    alt="avatar"
                ...
                <div className="relative z-10 flex flex-col justify-center items-start ps-3">
                  <div className="uppercase text-[11px] leading-[22px] tracking-wider font-normal text-card-foreground/80 group-hover:text-card-foreground/90">
                    {GLOBAL_RESPONSE?.UserTitle}
                  </div>
                  <div className="text-card-foreground/70 group-hover:text-card-foreground/80 text-[14px] leading-[20px] font-semibold">
                    {GLOBAL_RESPONSE?.customer_id
                      ? GLOBAL_RESPONSE.customer_name +
                        " " +
                        GLOBAL_RESPONSE.customer_surname
          ...
          contents={[
            ...
            },
            {
              ...
              className: "text-yellow-500 hover:bg-secondary",
            },
            GLOBAL_RESPONSE?.user_cizacl_role_id == "1" &&
              GLOBAL_RESPONSE?.pcp_user_id && {
                title: "History",
                className: "text-green-500 hover:bg-secondary",
                click: () => {
                  router.push("/history");
                },
              },
